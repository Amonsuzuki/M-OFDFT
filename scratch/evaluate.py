...
import scratch.ofdft.init
import scratch.ofdft.grad
...
def parse_args():
    ...
    parser.add_argument('--molecule', type=str, required=True, choices=['qm9.pbe.isomer', 'ethanol.pbe', 'qmugs.bin2_18.pbe', 'chgnolin.pbe'])
    parser.add_argument('--model-type', type=str, default='graphormer', choices=['graphormer', 'ofdft'])
    parser.add_argument('--use-ema', action='store_true')
    parser.add_argument('--init', type=str, required=True, choices=['minao', 'huckel'])
    parser.add_argument('--task-count', type=int, default=4, help='Number of tasks to be sampled. A number < 0 indicates evaluation on the whole set. Will be ignored if --task-list is specified.')
    parser.add_argument('--task-list', type=str, help='Path to a textfile containing ids of the tasks to be run, one per line. Overwrites --task-count if specified.')
    parser.add_argument('--task-id', type=int, help='Selected molecule index. Overwrites --task-count and --task-list if specified.')
    parser.add_argument('--bin-id', type=int, help='Selected bin index. Only valid when molecule type is qmugs.bin2-18')
    parser.add_argument('--ckpt-path', type=str, required=True)
    parser.add_argument('--reparam-spec', type=str, required=True)
    parser.add_argument('--prediction-type', type=str, required=True, choices=['Ts', 'Ts_res', 'TsExc', 'total'])
...
def get_tasks(args):
    if args.molecule == 'qmugs.bin2.18.pbe':
...
def get_init_coeff_and_energy(args, mol, auxmol):
    if args.init in ['minao', 'huckel']:
        init_dm = scratch.ofdft.init.get_init_coeff(f'initguess_{args.init}', mol, auxmol, use_dm=True)
    elif args.init == 'fastminao':
        init_dm = scratch.ofdft.init.get_init_coeff(f'initguess_{args.init}', mol, auxmol, use_dm=False)
    elif args.init.startswith('halfgt'):
        from scratch.ofdft.init import init_halfgt
        ...
        raise NotImplementedError()
    mf = mol.RKS(xc=XC)
    init_coeff = scratch.ofdft.init.get_rho_coeff(mol, auxmol, init_dm)
...
def get_tsbase_and_xc_and_forward_spec(args):
    ...
    if args.prediction_type == 'Ts':
        ...
        if args.ts_func == 'APBE':
            ...
            tsbase = {'LDA_K_TF': 1.0}
        tsbase = scratch.ofdft.functionals.build_tsxc(tsbase)
        xc = scratch.ofdft.functionals.build_tsxc(XC_C)
        forward_spec = ['tsbase', 'xc', 'corr', 'j', 'vext']
    elif args.prediction_type == 'TsExc':
        tsbase = scratch.ofdft.functionals.build_tsxc({'ZERO': 1.0})
        xc = scratch.ofdft.functionals.build_tsxc({'ZERO': 1.0})
        forward_spec = ['corr']
    else:
        raise NotImplementedError()
...
def worker(args, device, model, taskid):
    ...
    # get auxiliary basis and molecule for density fitting  
    auxbasis = scratch.ofdft.init.ref_etb(mol, 2.5)
    ...
    # initial guess of density matrix and corresponding energy
    ...
    # get ts reference, xc, and forward_spec == What to produce in later code
    ...
    if args.model_type == 'graphormer':
        ...
        from scratch.ofdft.drivers.mock import MoclOFDFTDriver
    ...
    # if considering total energy, adding nuclear energy to initial energy and ground truth energy
    ...
    # projection of grad matric to plain of N direction
    def grad_proj(grad):
        N = driver.norm_vec
        grad_proj = (grad - (grad @ N) / (N @ N) * N)
        return grad_proj
    
    # setting optimizer, momentum guarantees smooth direction.
    # velocity = momenutm * velocity + (1 - dampening) * gradient
    ...
    for i in range(args.steps):
        # recording coeff evolution in each steps
        if args.save_coeff_interval > 0:
            all_coeffs.append(driver.coeff_for_input.detach().cpu().numpy())

        # reset gradient, calculate gradient, return loss tensor and terms(tsbase, xc ...) and loss tensor to loss python float
        ...
        # set rho value and rho mae. grid_weights represents the size of grid region.
        if args.evaluate_rho:
            ...
            rho_ar_weighted_sum = (rho_ae @ grid_weights).sum().item()
        else:
            minrho = float('nan')
            maxrho = float('nan')
            rho_le0 = float('nan')
        ...
        # need to consider overlap matrix of orbitals
        if args.molecule in ['qm9.pbe.isomer', 'ethanol.pbe']:
        ...
        # record stats, terms is a response from driver
        stat_records.append({
            'step': i,
            'loss': loss_py,
            'min(rho)': minrho,
        ...
        if not args.quiet:
            print(f'[Step {i+1}] loss = {loss_py} min rho = {minrho} max rho = {maxrho} count(rho < 0) = {rho_le0} corr = {terms["corr"].detach().item()} projgradnorm = {total_grad_proj_norm} gradnorm = {total_grad_norm} rho_mae = {rho_mae} rho_ae_weighted_sum = {rho_ae_weighted_sum} coeff_mae = {coeff_mae} density_loss = {density_loss} hf_force_mae = {hf_force_mae}')
    ...
    # losses, delta_e is difference of adjacent elements, localminima, which has indices as array
    ...
    # extract energy total and hf force mae of the first local minima
    ...
    # calculate exponent moving average
    total_ema_delta_e = []
    ...
    # minimum EMA's energy and force
    ...
    # outputs back to the terminal
    sys.stdout = original_stdout
    new_stdout_file.close()

    # save coefficients, stop at minimum delta e, the first local minima, and projected the first local minima
    if args.save_coeff_interval > 0:
        coeffs_to_save = {
    ...
    # find best energy and force step(index) and loss(value) 
